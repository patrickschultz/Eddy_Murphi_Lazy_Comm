/* -*- C++ -*- 
 * splitFile.C
 * @(#) class for handling VERY large files
 *
 ...
 *
 */
splitFile::splitFile (unsigned long splitsize, bool permanent):
...
char *
splitFile::make_unique_filename (char *basepathname, bool with_protocol_name)
{
  ...
  do {
    if (with_protocol_name)
      sprintf (filenamebuffer, "%s_%s_%d.swp.%d",
	       basepathname, PROTOCOL_NAME, rand (), Communicate->GetRank());
    else
      sprintf (filenamebuffer, "%s_%d.swp,%d",
	       basepathname, rand (), Communicate->GetRank());
...
bool
splitFile::GotoNextPart (bool write_mode)
{
...
unsigned long
splitFile::getpartsize (int npart)
{
  /*
     if (npart == curpart)
...
bool
splitFile::open (char *filename, char *access)
{
...
unsigned long
splitFile::writebytes (void *buffer, unsigned long size)
{
...
bool
splitFile::seekbytes (unsigned long size, int direction, int &part,
		      unsigned long &skip)
{
  int i;
...
void
splitFile::seek (unsigned long size, unsigned long count, int from,
		 int direction)
{
  int part, i;
 ...
 */
unsigned long
splitFile::tell (unsigned long size)
{
