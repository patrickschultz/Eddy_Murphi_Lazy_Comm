...
void commManager::handleStatePullRequest()
{
  ...
  err = MPI_Recv(&requesting_rank, MESSAGE_REQ_STATES_LENGTH, MPI_UNSIGNED_LONG, MPI_ANY_SOURCE,
		  MESSAGE_REQ_STATES, MPI_COMM_WORLD, &stat);
  if (err){
...
bool commManager::ProcessMessages()
{
  ...
  err = MPI_Iprobe(MPI_ANY_SOURCE, MESSAGE_REQ_STATES,
		   MPI_COMM_WORLD, &flag, &status);
  if (err)
    {
      DisplayMPIError("MPI_Iprobe() in ProcessMessages()", err);
...
void commManager::RequestStates() {
	...
	int sendToRank = (myRank+1)%numProcs; //TODO change, this is for testing
