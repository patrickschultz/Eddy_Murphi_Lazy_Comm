...
cQueue::cQueue() :
	buf(0), numObjects(0), buf_state(BS_WACT), next(-1), max_buf_size(0),
			max_aux_len(0), state_len(0) {
}

cQueue::cQueue(size_t st_len, size_t buf_size, size_t aux_len) :
	numObjects(0), buf_state(BS_WACT), next(-1), max_buf_size(buf_size),
			max_aux_len(aux_len), state_len(st_len) {
...
commQueue::~commQueue(void) {
	for (int x = 0; x < NUMBER_OF_QUEUE_BUFFERS; x++)
		delete qArray[x];
	delete[] qArray;
}
...
/* called by commManager::PushState su queue[owner] */
{
	...
	//if Active is' active
	if (!qArray[Active]-> is_full()) { //if Active is not full
...
int commQueue::ReturnBuffer(int i) {
	...
	if (i < 0 || i > NUMBER_OF_QUEUE_BUFFERS)
		return retVal;
...
unsigned long commQueue::memory_allocated() {
	return (unsigned long) sizeof(commQueue)
			...
			+ (unsigned long) (qArray[0]->memory_allocated()
					* NUMBER_OF_QUEUE_BUFFERS);
}

//
// Total amount (in bytes) of memory allocated
//
unsigned long commQueue::memory_allocated(int BuffSize, int StateLen,
		int AuxLen, int NumBuffs) {
	return (unsigned long) sizeof(commQueue)
			+ (unsigned long) (sizeof(cQueue *) * NumBuffs)
			+ (unsigned long) (cQueue::memory_allocated(BuffSize, StateLen,
					AuxLen) * NumBuffs);
}
