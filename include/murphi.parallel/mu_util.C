...
/* IM<b> */
void
CheckMemAvailable (int BufSize, int BufCount, unsigned long bytes)
/* From Andreas\' code.*/
{
  double d_bytes = ((double)bytes - (double)Communicate->memory_allocated(BufSize, BufCount));
  if (Communicate->IsRoot() && d_bytes < 1000) 
    Error.Notrace("%.2lfMB are not enough, since %.2lfMB are required for the communication handling",
      ((double)bytes)/(1<<20), ((double)Communicate->memory_allocated(BufSize, BufCount))/(1<<20));
}
/* IM<e> */
...
{
  /* IM<b> */
  bytes = (unsigned long)((double)bytes - (double)Communicate->memory_allocated());
  /* IM<e> */
...
void
mu__int::boundary_error (int val) const
{
  Error.Error ("%d not in range for %s.", val, name);
}

int
mu__int::undef_error () const
{
  switch (category) {
  case STARTSTATE:
    Error.
  ...
  case RULE:
    Error.
      Error ("The undefined value at %s is referenced\n\tin rule:\n\t%s",
	     name, Rules->LastRuleName ());
  case INVARIANT:
    Error.
      Error ("The undefined value at %s is referenced\n\tin invariant:\n\t%s",
...
char *
mu_0_boolean::values[] = { "false", "true" };

void
mu_0_boolean::Permute (PermSet & Perm, int i)
{
};
void
mu_0_boolean::SimpleCanonicalize (PermSet & Perm)
{
};
void
mu_0_boolean::Canonicalize (PermSet & Perm)
{
};
void
mu_0_boolean::SimpleLimit (PermSet & Perm)
{
};
void
mu_0_boolean::ArrayLimit (PermSet & Perm)
{
};
void
mu_0_boolean::Limit (PermSet & Perm)
{
};
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
setofrules
interset (setofrules rs1, setofrules rs2)
{
  setofrules retval;
...
setofrules
different (setofrules rs1, setofrules rs2)
{
  setofrules retval;
...
bool
subset (setofrules rs1, setofrules rs2)
{
  for (unsigned r = 0; r < numrules; r++)
    if (rs1.in (r) && !rs2.in (r))
      return FALSE;
